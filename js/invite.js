...
(function(){
  ...
  if (to) {
    ...
    el.textContent = `Dear ${to}`; el.hidden = false;
  }
})();
...
form.addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    if (data.status==="success") {
      ...
      msg.style.color = "green"; form.reset(); currentPage=1; loadNotes();
    } else if (data.status==="duplicate") {
      msg.textContent = "⚠️ You already submitted recently."; msg.style.color = "#bb8a4e";
    } else if (data.status==="unauthorized" || data.status==="forbidden") {
      msg.textContent = "❌ Access denied. Please open from official link."; msg.style.color = "crimson";
    } else {
      msg.textContent = "❌ Gagal mengirim RSVP."; msg.style.color = "crimson";
    }
  }catch(err){
    msg.textContent = "❌ Error: " + err; msg.style.color = "crimson";
  }
});
...
aiForm.addEventListener("submit",(e)=>{
  ...
  userSay(t); botSay(reply(t)); aiText.value="";
});
